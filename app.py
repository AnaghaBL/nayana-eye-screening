    ...
    for case in reversed(my_cases):
        ...
        with st.expander(
            ...
            if status != "Reviewed":
                ...
                c1.link_button("PharmEasy", "https://pharmeasy.in", use_container_width=True)
                c2.link_button("1mg", "https://www.1mg.com", use_container_width=True)
                c3.link_button("Netmeds", "https://www.netmeds.com", use_container_width=True)

                st.write("")
                rx_text = f"""NAYANA — Prescription
----------------------
Patient      : {case['patient_name']}, {case['patient_age']}y, {case['patient_gender']}
Date         : {case.get('reviewed_at', '')}
Case ID      : {case['case_id']}

Diagnosis    : {case['doctor_diagnosis']}
Prescription : {case['doctor_prescription']}
Referral     : {case['doctor_referral']}
Notes        : {case['doctor_notes']}
AI Risk      : {case['risk_level']}

Generated by Nayana AI Tele-Ophthalmology.
Always consult your doctor before taking any medication."""
                st.download_button(
                    "⬇️ Download Prescription",
                    data=rx_text,
                    file_name=f"prescription_{case['case_id']}.txt",
                    mime="text/plain",
