...
def analyze_front_eye(image_pil):
    """
    Analyzes a front-eye image for 5 conditions.
    ...
    if circles is not None:
        ...
        if pupil_roi.size > 0:
            # Cloudy pupil = higher mean brightness
    ...
    # ── 5. Ptosis detection (drooping eyelid) ──────────────
    # Compare top vs bottom eye opening
...
def get_front_eye_recommendations(results):
    """Returns next steps based on analysis"""
    recommendations = []
    high_risk = []
        ...
        if score > 0.4:
            if condition == 'Redness / Conjunctivitis':
                recommendations.append(
                    "Possible conjunctivitis — avoid touching eyes, "
                    "consult a doctor if persists beyond 2 days"
                )
            elif condition == 'Swelling / Puffiness':
                recommendations.append(
                    "Eye swelling detected — apply cold compress, "
                    "seek medical attention if painful"
                )
            elif condition == 'Jaundice':
                recommendations.append(
                    "Yellowing of eyes detected — this may indicate "
                    "liver issues, consult a doctor immediately"
                )
            elif condition == 'Cataract':
                recommendations.append(
                    "Possible cataract — schedule a detailed eye "
                    "examination with an ophthalmologist"
                )
            elif condition == 'Drooping Eyelid (Ptosis)':
                recommendations.append(
                    "Eyelid drooping detected — consult a specialist "
                    "to rule out neurological causes"
                )

    needs_fundus = any(
        results.get(c, 0) > 0.5
        for c in ['Cataract', 'Drooping Eyelid (Ptosis)']
    )
